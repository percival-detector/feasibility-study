...
//        }
      ...
      // Calculate the time to wait until we hit the real start time
      double wait = 1.0 - (((double)startTime.nsec) / 1000000000.0);
      // Adjust according to our sub frame if we are in temporal mode
      if (mode != 0){
//        wait += ((postTime+0.063) * subFrameNumber);
        wait += ((postTime+0.001) * subFrameNumber);
      }
printf("Wait time: %lf\n", wait);
      if (wait > 0.001){
...
//epicsThreadSleep(0.04);
        //if (counter%8 == taskNumber){
          // Also send the reset data for the next frame, along with the number of sub frames and packet size
          senderPtr->sendImage(reset1_[(counter+8)%images], bufSize1_, 0, packetSize, counter+8, 1);
epicsThreadSleep(0.004);
          // Also send the reset data for the next frame, along with the number of sub frames and packet size
          senderPtr->sendImage(reset2_[(counter+8)%images], bufSize2_, 1, packetSize, counter+8, 1);
epicsThreadSleep(0.004);
          // Also send the reset data for the next frame, along with the number of sub frames and packet size
          senderPtr->sendImage(reset3_[(counter+8)%images], bufSize3_, 2, packetSize, counter+8, 1);
epicsThreadSleep(0.004);
          // Also send the reset data for the next frame, along with the number of sub frames and packet size
          senderPtr->sendImage(reset4_[(counter+8)%images], bufSize4_, 3, packetSize, counter+8, 1);
epicsThreadSleep(0.004);
          // Also send the reset data for the next frame, along with the number of sub frames and packet size
          senderPtr->sendImage(reset5_[(counter+8)%images], bufSize5_, 4, packetSize, counter+8, 1);
epicsThreadSleep(0.004);
          // Also send the reset data for the next frame, along with the number of sub frames and packet size
          senderPtr->sendImage(reset6_[(counter+8)%images], bufSize6_, 5, packetSize, counter+8, 1);
epicsThreadSleep(0.004);
          // Also send the reset data for the next frame, along with the number of sub frames and packet size
          senderPtr->sendImage(reset7_[(counter+8)%images], bufSize7_, 6, packetSize, counter+8, 1);
epicsThreadSleep(0.004);
          // Also send the reset data for the next frame, along with the number of sub frames and packet size
          senderPtr->sendImage(reset8_[(counter+8)%images], bufSize8_, 7, packetSize, counter+8, 1);
        //}
      }
